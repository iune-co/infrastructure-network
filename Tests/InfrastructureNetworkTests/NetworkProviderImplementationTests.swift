...
struct NetworkProviderImplementationRequestDataTests {
        ...
        struct ErrorHandlingTests {
                ...
                func testNoNetworkConnection() async {
                        ...
                        do {
                                // When
                                let _ = try await networkProvider.requestData(StubEndpoint.getEndpoint)
                ...
                func testInvalidURL() async {
                        ...
                        do {
                                // When
                                let _ = try await networkProvider.requestData(StubEndpoint.invalidURLEndpoint)
        ...
        struct ProcessingTests {
                ...
                func testSameNetworkSessionInstance() async {
                        ...
                        do {
                                // When
                                let _ = try await networkProvider.requestData(endpoint)
                                let _ = try await networkProvider.requestData(endpoint)
                                let _ = try await networkProvider.requestData(endpoint)
                ...
                func testQueryParameter() async {
                        ...
                        do {
                                ...
                                if let receivedURLRequest = networkSessionSpy.receivedURLRequest {
                                        // Then
                                        let receivedQueryParameters = TestHelper.getQueryItems(from: receivedURLRequest)
                                                .sorted(by: queryItemSorting)
                ...
                func testSuccessfulData() async {
                        ...
                        let ok200 = HTTPURLResponse.fixture(statusCode: 200)
                        let spy = NetworkSessionSpy(dataToReturn: payload, urlResponseToReturn: ok200)
                        let sut = NetworkProviderImplementation(networkSession: spy)
                        
                        do {
                                let data = try await sut.requestData(StubEndpoint.getEndpoint)
                                #expect(data == payload)
                        } catch {
...
fileprivate struct TestHelper {
        fileprivate static func getQueryItems(from request: URLRequest) -> [URLQueryItem] {
                guard
                        let url = request.url,
